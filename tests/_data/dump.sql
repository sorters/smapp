...
CREATE TABLE "purchaselines" ("id" integer not null primary key autoincrement, "state" tinyint(1) not null, "unit_price" float not null, "units" integer not null, "purchase_order_id" integer null, "provider_id" integer null, "product_id" integer not null, "created_at" datetime null, "updated_at" datetime null, foreign key("purchase_order_id") references "purchaseorders"("id"), foreign key("provider_id") references "providers"("id"), foreign key("product_id") references "products"("id"));
